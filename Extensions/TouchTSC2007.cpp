...
bool TFT_eSPI::initTSC2007(void) {
  return tsc2007.begin();
}
...
uint8_t TFT_eSPI::getTouchRaw(uint16_t *x, uint16_t *y){
  tsc2007.command(ACTIVATE_X, ADON_IRQOFF, ADC_12BIT);
  delay(1);
  *x = tsc2007.command(MEASURE_X, ADON_IRQOFF, ADC_12BIT);
  //delay(1);
  tsc2007.command(ACTIVATE_Y, ADON_IRQOFF, ADC_12BIT);
  delay(1);
  *y = tsc2007.command(MEASURE_Y, ADON_IRQOFF, ADC_12BIT);
...
uint16_t TFT_eSPI::getTouchRawZ(void){
  // Z sample request
  int16_t tz = 0xFFF;
  tz += tsc2007.command(MEASURE_Z1, ADON_IRQOFF, ADC_12BIT);
  tz -= tsc2007.command(MEASURE_Z2, ADON_IRQOFF, ADC_12BIT);
